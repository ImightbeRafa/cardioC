/**
 * BetsyCRM Integration
 * Syncs orders with retry logic (3 attempts, exponential backoff)
 */
...
async function sendOrderToBetsyWithRetry(order, maxRetries = 3) {
  ...
  for (let attempt = 1; attempt <= maxRetries; attempt++) {
    try {
      ...
      if (res.status >= 400 && res.status < 500) {
        ...
        console.error(`[BetsyCRM] Client error ${res.status} — not retrying:`, errorText);
        return null;
      }
  ...
  console.error('[BetsyCRM] All retries exhausted — order will not be synced');
  return null;
}
